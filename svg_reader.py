...
class SVG_READER(inkex.Effect):
                ...
                if len(parts) == 2:
                    ...
                    if prop == 'stroke':
                        ...
                        if simplestyle.isColor(col):
                            ...
                            new_val = '#' + \
                                self.colmod(c[0], c[1], c[2], path_id)
                        else:
                            new_val = col
                        if new_val != col:
    ...
    def Make_PNG(self):
        ...
        if self.inscape_exe is not None:
            try:
    ...
    def convert_text2paths(self):
        ...
        if self.inscape_exe is not None:
            try:
                ...
                self.document.write(svg_temp_file)
                cmd = [self.inscape_exe, "--export-text-to-path",
                       "--export-plain-svg", txt2path_file, svg_temp_file]
                run_external(cmd, self.timout)
...
# pass
        ...
        try:
            h_mm = self.unit2mm(self.document.getroot().xpath(
                '@height', namespaces=inkex.NSS)[0])
            w_mm = self.unit2mm(self.document.getroot().xpath(
        ...
        try:
            view_box_str = self.document.getroot().xpath(
