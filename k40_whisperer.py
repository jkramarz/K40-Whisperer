...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        top_File.add("command", label="Open Design (SVG/DXF)",
        ...
        top_USB.add("command", label="Initialize Laser",
...
##########################################################################
    ...
    def WriteConfig(self):
        ...
        header.append(
            "(=========================================================)")
        # BOOL
        header.append('(k40_whisperer_set include_Reng  %s )' %
                      (int(self.include_Reng.get())))
        header.append('(k40_whisperer_set include_Veng  %s )' %
                      (int(self.include_Veng.get())))
        header.append('(k40_whisperer_set include_Vcut  %s )' %
                      (int(self.include_Vcut.get())))
        header.append('(k40_whisperer_set halftone      %s )' %
                      (int(self.halftone.get())))
        header.append('(k40_whisperer_set HomeUR        %s )' %
                      (int(self.HomeUR.get())))

        # STRING.get()
        header.append('(k40_whisperer_set board_name    %s )' %
                      (self.board_name.get()))
        header.append('(k40_whisperer_set units         %s )' %
                      (self.units.get()))
        header.append('(k40_whisperer_set Reng_feed     %s )' %
                      (self.Reng_feed.get()))
        header.append('(k40_whisperer_set Veng_feed     %s )' %
                      (self.Veng_feed.get()))
        header.append('(k40_whisperer_set Vcut_feed     %s )' %
                      (self.Vcut_feed.get()))
        header.append('(k40_whisperer_set jog_step      %s )' %
                      (self.jog_step.get()))

        header.append('(k40_whisperer_set rast_step     %s )' %
                      (self.rast_step.get()))
        header.append('(k40_whisperer_set ht_size       %s )' %
                      (self.ht_size.get()))

        header.append('(k40_whisperer_set LaserXsize    %s )' %
                      (self.LaserXsize.get()))
        header.append('(k40_whisperer_set LaserYsize    %s )' %
                      (self.LaserYsize.get()))
        header.append('(k40_whisperer_set gotoX         %s )' %
                      (self.gotoX.get()))
        header.append('(k40_whisperer_set gotoY         %s )' %
                      (self.gotoY.get()))

        header.append('(k40_whisperer_set bezier_M1     %s )' %
                      (self.bezier_M1.get()))
        header.append('(k40_whisperer_set bezier_M2     %s )' %
                      (self.bezier_M2.get()))

        header.append('(k40_whisperer_set bezier_weight %s )' %
                      (self.bezier_weight.get()))

        header.append('(k40_whisperer_set t_timeout     %s )' %
                      (self.t_timeout.get()))
        header.append('(k40_whisperer_set n_timeouts    %s )' %
                      (self.n_timeouts.get()))

        header.append('(k40_whisperer_set designfile    \042%s\042 )' %
                      (self.DESIGN_FILE))
        header.append('(k40_whisperer_set inkscape_path \042%s\042 )' %
                      (self.inkscape_path.get()))
    ...
    def Entry_Reng_feed_Check(self):
        try:
            ...
            if value <= 0.0:
                self.statusMessage.set(
                    " Feed Rate should be greater than 0.0 ")
...
# return ERROR_cnt
    ...
    def Open_SVG(self, filemname):
        ...
        ###   Create ECOORDS   ###
        ##########################
        self.Vcut, self.Vcut_bounds = self.make_ecoords(
            svg_reader.cut_lines, scale=1 / 25.4)
        self.Veng, self.Veng_bounds = self.make_ecoords(
            svg_reader.eng_lines, scale=1 / 25.4)
        ...
        if (self.Reng_image is not None):
    ...
    def make_ecoords(self, coords, scale=1):
        ...
        for line in coords:
            ...
            # check and see if we need to move to a new discontinuous start
            # point
...
# return grey
    ...
    def make_raster_coords(self):
        ...
        if (self.Reng_image is not None):
            ...
            for i in range(0, self.him, Raster_step):
                self.statusMessage.set(
                    "Raster Engraving: Creating Scan Lines: %.1f %%" % ((100.0 * i) / self.him))
                self.master.update()
                if self.stop[0] == True:
    ...
    def convert_halftoning(self, image):
        ...
        if w > 0:
            ...
            for y in range(1, y_lim):
                self.statusMessage.set(
                    "Raster Engraving: Adjusting Image Darkness: %.1f %%" % ((100.0 * y) / y_lim))
                self.master.update()
                for x in range(1, x_lim):
...
# image.save("Z:\\002.png","PNG")
    ...
    def Open_DXF(self, filemname):
        ...
        self.Vcut, self.Vcut_bounds = self.make_ecoords(
            dxf_cut_coords, scale=dxf_scale)
        self.Veng, self.Veng_bounds = self.make_ecoords(
            dxf_engrave_coords, scale=dxf_scale)
        ...
        for line in fin:
            if ident in line:
                ...
                if "include_Reng" in line:
                    self.include_Reng.set(
                        line[line.find("include_Reng"):].split()[1])
                elif "include_Veng" in line:
                    self.include_Veng.set(
                        line[line.find("include_Veng"):].split()[1])
                elif "include_Vcut" in line:
                    self.include_Vcut.set(
                        line[line.find("include_Vcut"):].split()[1])
                ...
                # STRING.set()
                    ...
                    self.bezier_M1.set(
                        line[line.find("bezier_M1"):].split()[1])
                elif "bezier_M2" in line:
                    self.bezier_M2.set(
                        line[line.find("bezier_M2"):].split()[1])
                elif "bezier_weight" in line:
                    self.bezier_weight.set(
                        line[line.find("bezier_weight"):].split()[1])

                elif "t_timeout" in line:
                    self.t_timeout.set(
                        line[line.find("t_timeout"):].split()[1])
                elif "n_timeouts" in line:
                    self.n_timeouts.set(
                        line[line.find("n_timeouts"):].split()[1])
...
# self.master.update()
    ...
    def write_egv_to_file(self, data):
        try:
            ...
            self.statusMessage.set(
                "Unable to open file for writing: %s" % (fname))
            return
        # ctog="B"
    ...
    def menu_Help_Web(self):
        webbrowser.open_new(
            r"http://www.scorchworks.com/K40whisperer/k40whisperer.html")
    ...
    def Master_Configure(self, event, update=0):
        ...
        if abs(self.w - w) > 10 or abs(self.h - h) > 10 or update == 1:
            ...
            if 0 == 0:
                ...
                self.Up_Button.place(x=xoffst + 12 + bsz,
                                     y=Yloc, width=bsz, height=bsz)
                self.UR_Button.place(x=xoffst + 12 + bsz * 2,
                                     y=Yloc, width=bsz, height=bsz)
                Yloc = Yloc + bsz
                self.Left_Button.place(
                    x=xoffst + 12, y=Yloc, width=bsz, height=bsz)
                self.CC_Button.place(x=xoffst + 12 + bsz,
                                     y=Yloc, width=bsz, height=bsz)
                ...
                self.LR_Button.place(x=xoffst + 12 + bsz * 2,
                                     y=Yloc, width=bsz, height=bsz)
                ...
                self.PreviewCanvas.configure(
                    width=self.w - (220 + 20), height=self.h - 50)
    ...
    def Plot_Data(self):
        ...
        if self.Reng_image is not None:
            if self.include_Reng.get():
                ...
                self.Plot_Raster(self.laserX, self.laserY, x_lft,
                                 y_top, self.PlotScale, im=self.UI_image)
        else:
            self.UI_image = None
        ######################################
        ###       Plot Veng Coords         ###
...
# yold=y1
        ...
        ######################################
        dot_col = "grey50"
        self.Plot_circle(self.laserX, self.laserY, x_lft,
                         y_top, self.PlotScale, dot_col, radius=5)
    ...
    def GEN_Settings_Window(self):
        ...
        self.Label_N_Timeouts.place(
            x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Entry_N_Timeouts = Entry(gen_settings, width="15")
        self.Entry_N_Timeouts.place(
            x=xd_entry_L, y=D_Yloc, width=w_entry, height=23)
        ...
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-M1", state="disabled")
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-M", state="disabled")
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-B2", state="disabled")
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-B1", state="disabled")
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-B", state="disabled")
        self.Board_Name_OptionMenu['menu'].entryconfigure(
            "LASER-A", state="disabled")
        ...
        self.Label_SaveConfig.place(
            x=xd_label_L, y=D_Yloc, width=113, height=21)
        ...
        self.GEN_Close = Button(
            gen_settings, text="Close", command=self.Close_Current_Window_Click)
    ...
    def RASTER_Settings_Window(self):
        ...
        self.BezierCanvas.create_line(
            5, 260 - 0, 260, 260 - 255, fill="grey", capstyle="round", width=2, tags='perm')
        ...
        self.RASTER_Close = Button(
            raster_settings, text="Close", command=self.Close_Current_Window_Click)
